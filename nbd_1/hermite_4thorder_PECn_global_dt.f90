...
if ( integration_loop_counter.eq. 0 ) then
   ...
   if ( (global_time + dt) .gt. time_at_next_snapshot ) then
      ...
      print *, "HERMITE: dt limited by snapshot time at startup!", global_time, dt
   end if   
end if
...
do n_iter = 1, n_order
   ...
   if (n_iter .eq. 1 ) then
      ...
      call get_accel_info(N, xc, yc, zc, vxc, vyc, vzc, m, soft, ax1, ay1, az1, axdot1, aydot1, azdot1, radii)
   end if
...
if (dt .lt. dt_min) then
   print *, "HERMITE: preventing dt from falling below dt_min", dt, dt_min
   dt = dt_min
end if
