...
def nbd_read_ascii():
  #
  # load empty class struct to fill with data
  data = nbd_data_struct()
  #
  # open file and unpack data into class struct
  data.time, data.dt, data.eng, data.asemi, data.per, data.ecc, data.rrel, data.vrel = np.loadtxt('binary_parameters.dat', dtype='float64', unpack=True, delimiter=' ')
  #
  # return struct
  return data
